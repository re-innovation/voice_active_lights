...
void update_leds()
{
  ...
  for (int i = 0; i < NUM_LEDS; i++)
  {
    // update to most recent start times
    if (i < leds_switched_on && my_led_array[i].start_time > )
    {
      my_led_array[i].start_time = recent_start_time;
    }
  }
  ...
  for (int p = 0; p < NUM_LEDS; p++)
  {
    ...
    if (my_led_array[p].led_active == true)
    {
      ...
      else if (led_time >= FADE_IN_TIME && led_time < (FADE_IN_TIME + TWINKLE_TIME))
      {
        if (p < leds_switched_on)
        {
          // In this case we want to keep on twinkling
          my_led_array[p].start_time = now - FADE_IN_TIME - 1;  // Puts it into twinkling time slot (start of it)
        }
      }
    ...
    else if (my_led_array[p].led_active == true && (led_time < (FADE_IN_TIME + TWINKLE_TIME))) //my_led_array[p].start_time
    {
      // Twinkle the LEDs
