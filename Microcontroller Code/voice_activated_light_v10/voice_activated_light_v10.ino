...
void update_leds()
{
  ...
  for (int p = 0; p < NUM_LEDS; p++)
  {
    ...
    // re-calculate the new led_times:
    led_time = now - my_led_array[p].start_time; // Calculate difference between now and start time
    ...
    if (my_led_array[p].led_active == true && (led_time < FADE_IN_TIME) && my_led_array[p].twinkle_flag == false)
    {
    ...
    else if ((my_led_array[p].led_active == true && (led_time < (FADE_IN_TIME + TWINKLE_TIME))))
    {
      ...
      // Twinkle the LED section 2
      // Want a random delay then randon time for sparkle (up to a point) and random colour.
      if (millis() > my_led_array[p].twinkle_timer_2 && my_led_array[p].twinkle_flag_2 == false)
      {
        leds.setPixelColor(NUM_LEDS + p, leds.Color(colors[3][0], colors[3][1], colors[3][2]));
        my_led_array[p].twinkle_flag_2 = true;
        my_led_array[p].twinkle_timer_2 = millis() + random(TWINKLE_SPARK_MIN, TWINKLE_SPARK_MAX);
      }
      if (millis() > my_led_array[p].twinkle_timer_2 && my_led_array[p].twinkle_flag_2 == true)
      {
        leds.setPixelColor(NUM_LEDS + p, leds.Color(colors[2][0], colors[2][1], colors[2][2]));
        my_led_array[p].twinkle_flag_2 = false;
        my_led_array[p].twinkle_timer_2 = millis() + random(TWINKLE_DELAY_MIN, TWINKLE_DELAY_MAX);
      }
      // Twinkle the LED section 3
      // Want a random delay then randon time for sparkle (up to a point) and random colour.
      if (millis() > my_led_array[p].twinkle_timer_3 && my_led_array[p].twinkle_flag_3 == false)
      {
        leds.setPixelColor(2 * NUM_LEDS + p, leds.Color(colors[5][0], colors[5][1], colors[5][2]));
        my_led_array[p].twinkle_flag_3 = true;
        my_led_array[p].twinkle_timer_3 = millis() + random(TWINKLE_SPARK_MIN, TWINKLE_SPARK_MAX);
      }
      if (millis() > my_led_array[p].twinkle_timer_3 && my_led_array[p].twinkle_flag_3 == true)
      {
        leds.setPixelColor(2 * NUM_LEDS + p, leds.Color(colors[4][0], colors[4][1], colors[4][2]));
        my_led_array[p].twinkle_flag_3 = false;
        my_led_array[p].twinkle_timer_3 = millis() + random(TWINKLE_DELAY_MIN, TWINKLE_DELAY_MAX);
      }
    }
